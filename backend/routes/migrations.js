...
/**
 * POST /api/migrations/prompt-sections-to-templates
 * Migrate hardcoded prompt sections to SystemPromptTemplate entries
 * SUPER_ADMIN only
 */
router.post('/prompt-sections-to-templates', authenticate, requireRole('SUPER_ADMIN'), async (req, res) => {
  try {
    const templates = [];
    ...
    for (const template of templates) {
      try {
        ...
        skipped++;
        results.push({ name: template.name, action: 'error', error: error.message });
      }
    }
